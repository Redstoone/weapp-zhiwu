...
Page({
  ...
  getCarts() {
    App.HttpService.getCartByUser().then(res => {
      const data = res.data;
      console.log(data);
      if (data.meta.code == 0) {
        data.data.forEach(
          n =>
            (n.goods.thumb_url = App.renderImage(
              n.goods.images[0] && n.goods.images[0].path
            ))
        );
        this.setData({
  ...
  del(e) {
    ...
    App.WxService
      ...
      .then(data => {
        if (data.confirm == 1) {
          App.HttpService.delCartByUser(id).then(res => {
            const data = res.data;
            console.log(data);
            if (data.meta.code == 0) {
              this.getCarts();
  ...
  putCartByUser(id, params) {
    App.HttpService.putCartByUser(id, params).then(res => {
      const data = res.data;
      console.log(data);
      if (data.meta.code == 0) {
        this.getCarts();
